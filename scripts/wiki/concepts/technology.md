...
- The **technology LEF** (`asap7_tech_1x_201209.lef`) describes the *process* as routers see it: the metal/via stack (M1–M9 + Pad on the stock stack), each layer's preferred routing direction, widths/spacings/pitches and advanced rules (e.g. the LEF58 end-of-line keep-outs that appear in routing), via definitions, the manufacturing grid, and the placement **site** — for `asap7sc7p5t` a 0.054 × 0.270 µm tile, the atomic unit of placement.
