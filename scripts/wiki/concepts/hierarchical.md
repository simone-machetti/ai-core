...
**PDN** (`scripts/pnr/pdn_macro.tcl`, macro grid section): the layer-matching duty. Blocks hardened by this flow (`MAX_ROUTE_LAYER=M5` with `pdn_tile.tcl`) export their **M5** power straps as PG pins and leave M6/M7 empty, so the parent's M6 mesh runs over the macros and is dropped onto them:
...
```bash
# 1. Harden each block: routing capped at M5, tile PDN with M5 pins, and
#    optionally a pin plan (edge/span/order per bus) and I/O budgets
make pnr TOP_LEVEL=<block> CLK_PERIOD_NS=<t> OUT_DIR=<block_pnr> NETLIST_DIR=<block_syn> \
    MAX_ROUTE_LAYER=M5 PDN=scripts/pnr/pdn_tile.tcl [PINS=<pins.tcl>] [SDC=<budgets.tcl>]
...
# 3. Parent P&R with macros (channels between macro columns/rows, its own
#    pin plan aligned with the placed macros, optionally M8/M9 over the tiles)
...
- **Pin planning**: a block's pin placement quality becomes the *parent's* routing problem multiplied by instance count. The block's `PINS` file (one ordered group per bus, each on the edge the parent reaches it from) and `PIN_LAYERS_HOR/VER` (two layers on a bus-heavy edge) fix that geometry at hardening time; the parent's own `PINS` file reads the placed macros and puts each row's or column's bus on the boundary at that row's or column's span.
...
- **Boundary optimization stops at the wall**: paths through a macro use its frozen arcs; the parent can buffer *up to* the pins only. Generous input margins at hardening time are the block designer's courtesy to the integrator — declared with an `SDC` file of per-port input/output delays (a share of the period the parent may spend before the pin). An input whose internal path is too deep to take a budget at all (a control that gates a whole tree right after the pin) leaves the parent nothing; such pins need a register stage in the RTL, not a P&R knob.
...
Source: [run.tcl](../../syn/run.tcl) — [init_tech.tcl](../../pnr/init_tech.tcl) — [1_floorplan.tcl](../../pnr/1_floorplan.tcl) — [pdn_tile.tcl](../../pnr/pdn_tile.tcl) — [pdn_macro.tcl](../../pnr/pdn_macro.tcl) — [6_gds.sh](../../pnr/6_gds.sh) — Reference: [asic_flow.md](../../asic_flow.md) — Index: [index.md](../index.md)
