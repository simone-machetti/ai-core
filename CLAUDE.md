...
This derives `REPO_HOME` (the repo root) from the script's own location, then sources your `~/.bashrc` — where you export the tool/PDK install **roots**: `EDA_HOME` (puts Verilator, Yosys, Yosys-Slang and OpenSTA on `PATH`) and `PDK_HOME`. `PDK_HOME` is the checkout of the ASAP7 platform repository (`asap7-smic-n3-beol`); the Makefile derives `ASAP7_HOME`, the cell library, as `$PDK_HOME/vendor/asap7`, and `BEOL_HOME`, the metal stack read by place-and-route, as `$PDK_HOME/beol/$BEOL`, with `BEOL=asap7` (the stock stack, same tree as the cells) by default and `smic-n3` as the restacked variant. The repo-root variable is intentionally generic — the shared flow hardcodes no project or repo name — so paths inside the flow are resolved as `$REPO_HOME/projects/$PROJECT/...`.
