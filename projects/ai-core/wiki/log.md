...
- **[Creation]**: Completed the PE — added [pe_ctrl](architecture/pe_ctrl.md) (combinational mode decoder: a `mode_i`-indexed lookup table driving every datapath control; `prop_carry` derived from `sel_out`), [pe_datapath](architecture/pe_datapath.md) (structural `disp_array → pe_array → acc_array` wrapper, taps internal), and [pe_top](architecture/pe_top.md) (`pe_ctrl` + `pe_datapath` plus the control-path pipeline registers that delay each control to its stage: input reg for `mode`/`sel_acc`/`acc`, group-2 reg for `sel_shift[2:1]`/`sel_out`/`prop_carry`, group-3 reg for `sel_acc`/`acc`; `sel_shift` reassembled with bit 0 combinational). Verified whole-PE across all 11 modes, single-shot and accumulating, driven only by `mode_i`/operands/`sel_acc`/`acc_i`, with [tb_pe_top](verification/pe_top.md). Added the three architecture pages, the verification page, and the `index.md` entries.
- **[Fix]**: `doc/diagrams/pe_top.md` — refreshed for the built RTL: `ctr_l`/`ctr_h` are 2-bit (pass/zero/negate) per pair, the internal tap-level select is `sel_out` (dropped the `sel_tap` working name), the pipeline is a confirmed 3-stage (disp input / `pe_array` L0 / `acc_array` output) with `pe_ctrl` combinational and control alignment in `pe_top`, and `acc_i` is an external accumulator input. Updated the `primitives.md` status line (all sub-blocks and `pe_top` now built).
