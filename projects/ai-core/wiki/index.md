...
* [Processing Element (Bit-Plane-A BFP)](modules/pe_bpl_a_bfp.md) — `pe_bpl_a_bfp`: the bit-plane PE core — `pe_array_bpl_a_bfp` + `acc_array_bpl_bfp` + twin acc/acc-exp regs; `en_i` masks **five** buses (the pair sums are a third operand); no `is_signed_b_i`, resolved upstream. 5148.90 µm², −3.7 % vs `pe_bfp`.
...
* [Dispatch Array B (Bit-Plane-A BFP)](modules/disp_array_b_bpl_a_bfp.md) — `disp_array_b_bpl_a_bfp`: bit-plane B dispatch — `disp_array_b` routing plus one `gate_n_bpl_bfp` per DP8, so the column broadcasts 8 × 5-bit exact signed values **and** 4 × 6-bit pairwise sums; consumes `is_signed_b` so the PEs never see it.
...
* [PE Array (Bit-Plane-A BFP)](modules/pe_array_bpl_a_bfp.md) — `pe_array_bpl_a_bfp`: `pe_array_bfp`'s tree with `dp_8_bpl_a_bfp` leaves — same 11 aligners and max-tree; `+b_sum_dp8_i`, no `is_signed_b_i`; nodes 31/36/44/44 (a guard bit at L0 and L1), taps 18/36/40/40.
...
* [Dot Product 8 (Bit-Plane-A BFP)](modules/dp_8_bpl_a_bfp.md) — `dp_8_bpl_a_bfp`: bit-plane DP8 — 32 multiplexers selected by A's bit planes over precomputed B partials instead of Booth recoding; 22-bit sign-consistent carry-save, no `is_signed_b_i`. 190.63 µm² standalone, −5.8 % vs `dp_8`.
...
* [Gate N (Bit-Plane BFP)](modules/gate_n_bpl_bfp.md) — `gate_n_bpl_bfp`: bit-plane operand gate — widens each word to its exact signed value (`WIDTH+1`) and emits the `SIZE/2` pairwise sums (`WIDTH+2`); lives in the column dispatch so the adders amortize over the column.
...
* [tb_top_NxN_bpl_a_bfp](testbenches/tb_top_NxN_bpl_a_bfp.md) — the bit-plane-A BFP grid streaming at full throughput, distinct A/row + B/col; each mode × 3 patterns × 2 exponent experiments (equal-exp bit-exact matmul + distinct-exp exponent/mantissa window), **66/66**, N=2, 0 mismatches.
...
* [tb_pe_array_bpl_a_bfp](testbenches/tb_pe_array_bpl_a_bfp.md) — `disp_array_b_bpl_a_bfp → pe_array_bpl_a_bfp` with the baseline `pe_array` alongside; Pass A equal-exp taps carry the same **value** as the integer tree (the carry-save encoding legitimately differs), Pass B distinct-exp max-tree + window, all 11 modes.
...
* [tb_acc_array_bpl_a_bfp](testbenches/tb_acc_array_bpl_a_bfp.md) — whole bit-plane-A BFP path (`disp → pe_array_bpl_a_bfp → acc_array_bpl_bfp`) vs the integer path + matmul golden; Pass A equal-exp agreement at `pe_out` on **all 8 lanes**, single-shot + accumulate, Pass B min-scale-seed closed form, all 11 modes.
...
* [tb_disp_array_bpl_a_bfp](testbenches/tb_disp_array_bpl_a_bfp.md) — the bit-plane dispatch pair (`disp_array_a` + `disp_array_b_bpl_a_bfp`) vs a golden of the whole chain: route, H/L split, gate **with the cross-half negate carry**, signed widening, pair sums; all 11 modes on `ctrl`'s real control vectors.
...
* [tb_dp_8_bpl_a_bfp](testbenches/tb_dp_8_bpl_a_bfp.md) — resolve + sign-consistency + value-equivalence against `dp_8`, all four signedness combos; corner-biased because the tightest compressor stage sits only ~6 % inside the sign-consistency bound.
