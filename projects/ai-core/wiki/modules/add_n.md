...
There is no signedness control. `add_n` treats its inputs as unsigned; when it resolves a signed value, the sign is carried by upstream sign-extension (in [acc_array](../architecture/acc_array.md) each tap is sign-extended to 40 bits and windowed before the lane adds), so the plain unsigned add with the carry chain reconstructs the correct signed result across the fused lanes.
