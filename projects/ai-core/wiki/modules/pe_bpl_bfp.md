# Processing Element (Bit-Plane BFP)

`pe_bpl_bfp` — the bit-plane variant of the per-PE core [pe_bfp](./pe_bfp.md). Same shape and same 3-stage timing: operand isolation → [pe_array_bpl_bfp](./pe_array_bpl_bfp.md) → [acc_array_bpl_bfp](./acc_array_bpl_bfp.md), plus the twin acc / acc-exp pipeline registers.
...
2. **No `is_signed_b_i`.** B's signedness is resolved once per column in [disp_array_b_bpl_bfp](./disp_array_b_bpl_bfp.md), so the flag never reaches the PE.
...
Exercised end to end by [tb_top_NxN_bpl_bfp](../testbenches/tb_top_NxN_bpl_bfp.md) at the grid level; its two halves are proven separately by [tb_pe_array_bpl_bfp](../testbenches/tb_pe_array_bpl_bfp.md) and [tb_acc_array_bpl_bfp](../testbenches/tb_acc_array_bpl_bfp.md).

Source: [pe_bpl_bfp.sv](../../rtl/pe_bpl_bfp.sv) — Diagram: [pe_bpl_bfp](../../doc/diagrams/pe_bpl_bfp.excalidraw)
