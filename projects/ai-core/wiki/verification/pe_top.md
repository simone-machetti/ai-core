...
`tb_pe_top` verifies the whole PE, [pe_top](../architecture/pe_top.md), driven **only** by its external interface — the two operands, `mode_i`, `sel_acc_i` and `acc_i`. Because none of the internal `sel_*` / `ctr_*` controls are driven by the tb, a passing run proves not just the datapath but also that [pe_ctrl](../architecture/pe_ctrl.md) decodes `mode_i` into the right controls and that `pe_top` delays each of them to the correct pipeline cycle. Each of the 11 modes is checked at `pe_out` as a plain matrix multiply `X = A · B`, reusing the independent golden model from `tb_pe_array`. Lanes are corner-biased so the sign-consistency corners are reached.
...
```
make sim PROJECT=ai-core TOP_LEVEL=pe_top PARAMS="NUM_RAND=2000"
```
...
| Alignment    | a wrong per-stage control delay in `pe_top` misaligns a control with its data — also a mismatch                            |
...
Errors accumulate in `err`. Each mode prints `PASS` / `FAIL` for both passes; the run ends with `pe_top verification: M/11 single-shot modes passed (E total mismatches)` — a clean run reports `11/11 ... 0 total mismatches` and every `acc mode N: PASS`.

Source: [tb_pe_top.sv](../../tb/tb_pe_top.sv) — DUT: [pe_top](../architecture/pe_top.md)
