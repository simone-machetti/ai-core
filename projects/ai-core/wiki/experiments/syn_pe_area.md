# Intra-PE Area — Baseline / Square / Baseline-BFP / Square-BFP / Bit-Plane-A BFP / Bit-Plane-B BFP

Cell-area breakdown *inside* one processing element for the six variants — [pe](../modules/pe.md), [pe_sqr](../modules/pe_sqr.md), [pe_bfp](../modules/pe_bfp.md), [pe_sqr_bfp](../modules/pe_sqr_bfp.md), [pe_bpl_a_bfp](../modules/pe_bpl_a_bfp.md) and [pe_bpl_b_bfp](../modules/pe_bpl_b_bfp.md) — split into the DP8 array, the compression tree and the accumulator.
...
These four net figures, **−13.0 %**, **−2.6 %**, **−3.7 %** and **−14.2 %**, are the asymptotes of [Grid Scaling](syn_scaling.md): they are what each variant achieves once its per-row/per-column overhead is amortized away.

## Caveats

Preserving a module boundary blocks cross-boundary optimization, so the hierarchical total runs **above** the flat PE area used by every other experiment — **9–17 %** across the six, with bit-plane A the most affected (+17.4 %) and bit-plane B the least (+9.4 %) — a signal of how much cross-boundary optimization each has to lose at the DP8/tree interface. The composition is measured, but the totals are not directly comparable with the flat figures — which is why the normalized table scales each variant's shares to its flat total. Quote the two separately and never substitute one for the other.
