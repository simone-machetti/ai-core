# PE Array (Bit-Plane BFP) Testbench

## Purpose

`tb_pe_array_bpl_bfp` verifies [pe_array_bpl_bfp](../modules/pe_array_bpl_bfp.md) wired downstream of [disp_array_b_bpl_bfp](../modules/disp_array_b_bpl_bfp.md) and the BFP exponent dispatchers, with the baseline [pe_array](../modules/pe_array.md) — fed by the ordinary [disp_array_b](../modules/disp_array_b.md) — instantiated **alongside** as the integer reference.
...
```
make sim PROJECT=ai-core TOP_LEVEL=pe_array_bpl_bfp
```
...
The bench packs the per-block source exponents (4 × 6-bit A word, 4 × 12-bit B word); [disp_array_exp_a_bfp](../modules/disp_array_exp_a_bfp.md) / [disp_array_exp_b_bfp](../modules/disp_array_exp_b_bfp.md) dispatch them to the two 6-bit per-DP8 inputs, and `pe_array_bpl_bfp` forms the 7-bit scales. A software sideband model runs alongside as the golden and the dispatcher outputs are checked against it every vector.

Result: **11/11 modes, 0 mismatches**.

Source: [tb_pe_array_bpl_bfp.sv](../../tb/tb_pe_array_bpl_bfp.sv) — DUT: [pe_array_bpl_bfp](../modules/pe_array_bpl_bfp.md) (through [disp_array_a](../modules/disp_array_a.md) + [disp_array_b_bpl_bfp](../modules/disp_array_b_bpl_bfp.md) + the BFP exponent dispatchers; reference [pe_array](../modules/pe_array.md))
