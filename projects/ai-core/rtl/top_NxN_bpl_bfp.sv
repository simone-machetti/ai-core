...
//
// Description:
//   BFP N x N grid of Processing Elements, bit-plane build - the top_NxN_bfp
//   variant built on the bit-plane column dispatch and PE (disp_array_b_bpl_bfp,
//   pe_bpl_bfp). Tiles N*N cores; operand A (mantissa + exponent) is shared per
//   row and operand B per column, so PE[r][c] evaluates A[r] . B[c] in block
//   floating point, with the same values as top_NxN_bfp.
//
//   Each column's dispatcher also emits the per-DP8 pairwise B sums the
//   bit-plane cores consume, so those adders are paid once per column rather
//   than once per PE. Because the dispatcher resolves B to signed values,
//   ctrl's is_signed_b feeds the N dispatchers and never reaches the PEs.
//
//   is_signed_a is masked here for the DP8s the mode leaves idle, taken from
...
//
//   Everything else is top_NxN_bfp: ctrl, disp_array_a, both exponent
...
module top_NxN_bpl_bfp #(
    ...
    logic                 is_signed_a_g [ 0:NUM_DP8-1];
    ...
    logic [ B_SUM_WIDTH-1:0] b_sum_dp8_col [0:NUM_COL-1][0:NUM_DP8-1];
    ...
    generate
        for (r = 0; r < NUM_ROW; r++) begin : gen_pe_row
            for (c = 0; c < NUM_COL; c++) begin : gen_pe_col
                ...
                pe_bpl_bfp pe_bpl_bfp_i (
                    .clk_i        (clk_pe),
                    .rst_ni       (rst_ni),
                    .a_dp8_i      (a_dp8_row[r]),
                    .b_dp8_i      (b_dp8_col[c]),
                    .b_sum_dp8_i  (b_sum_dp8_col[c]),
                    .exp_a_dp8_i  (exp_a_dp8_row[r]),
                    .exp_b_dp8_i  (exp_b_dp8_col[c]),
                    .en_i         (en_pe),
                    .is_signed_a_i(is_signed_a_g),
