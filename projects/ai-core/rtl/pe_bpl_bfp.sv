...
//
// Description:
//   BFP Processing Element core, bit-plane build - the pe_bfp variant built on
//   pe_array_bpl_bfp and acc_array_bpl_bfp. Same shape as pe_bfp: operand
//   isolation, then the array, then the accumulator, plus the acc and acc-exp
//   pipeline registers, and the same 3-stage timing.
//
//   The dispatched B arrives as 8 x 5-bit exact signed lane values plus 4 x
//   6-bit pairwise sums, so en_i AND-masks five buses - a_dp8, b_dp8, b_sum_dp8,
//   exp_a_dp8 and exp_b_dp8 - keeping a gated PE quiet at the bit-plane
//   multiplexer inputs as well as its selects. is_signed_b_i is not needed here,
//   since B's signedness is resolved in the dispatcher.
//
...
module pe_bpl_bfp #(
    ...
    logic [ B_SUM_WIDTH-1:0] b_sum_dp8_m [0:NUM_DP8-1];
