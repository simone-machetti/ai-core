...
//
// Description:
//   Variant of cpr_tree used exclusively by top_sqr_8x8_alpha. Processes
//   PP_SIZE = 16 partial products (no accumulator inputs) through two
//   compression stages. Stage 0 groups PP_SIZE/2 = 8 inputs per lane and
...
//   PP_WIDTH + CPR_EXT_BITS + 16. The pipeline register is placed after
//   stage 1 (the last compression stage) when IS_PIPELINE = 1.
//
// Parameters:
//   IS_PIPELINE - 1 = insert pipeline register after stage 1; 0 = no register
//   PP_SIZE     - number of partial product inputs (must be a multiple of 2)
...
module cpr_tree_8x8_alpha #(
    parameter bit IS_PIPELINE = 1,
    parameter int PP_SIZE     = 16,
    parameter int PP_WIDTH    = 8,
    ...
    function automatic int gen_in_width(input int stage);
        ...
        begin
            if (stage == 0) begin
                ...
                gen_in_width = PP_WIDTH + CPR_EXT_BITS + 8;
            end else if (stage == 2) begin
                tmp          = PP_WIDTH + CPR_EXT_BITS + 8;
                gen_in_width = tmp + 8;
            end else begin
                gen_in_width = PP_WIDTH;
            end
    ...
    generate
        ...
        for (stage = 0; stage < NUM_STAGES; stage++) begin
            ...
            localparam int NUM_LANES            = 2 / pow2(stage);
        ...
        localparam int ADD_N_IN_WIDTH  = gen_in_width(2);
        ...
        assign add_n_sum   = tmp[2][0][ADD_N_IN_WIDTH-1:0];
        assign add_n_carry = tmp[2][1][ADD_N_IN_WIDTH-1:0];
