...
//
// Description:
//   B-exponent dispatch array for the square variant - the BFP exponent
//   sideband counterpart of disp_array_b_sqr, shared by a whole grid column
//   (one instance per column, next to the mantissa dispatcher it mirrors).
//   Routes the 4 per-block B exponent chunks (one 12-bit chunk per 64-bit B
//   block: the exponents of its two 32-bit halves, since plane packing can put
//   two halves with different exponents in one block) to the 16 DP8s using the
//   same 4->1 block select per pair and the same fixed high/low split as the
//   mantissa path: chunk bits [11:6] are the exponent of the block's high (H)
//   half and go to the even DP8 (2p), bits [5:0] belong to the low (L) half and
//   go to the odd DP8 (2p+1).
//
//   Same structure as disp_array_exp_b_bfp, with one change: the idle mask is
...
//   baseline per-pair ctr_h_i / ctr_l_i ZERO-decode. zero_i[2p] gates the H
//   half, zero_i[2p+1] the L half.
//
//   Gating the exponent - not only the mantissa - is still required: even
//   though disp_array_b_sqr already zeroes an idle DP8's mantissa, the exponent
...
//   operand register of disp_array_b_sqr; the dispatch is combinational and its
//   output is broadcast to the column's PEs.
// -----------------------------------------------------------------------------

`timescale 1 ns/1 ps

module disp_array_exp_b_sqr_bfp #(
