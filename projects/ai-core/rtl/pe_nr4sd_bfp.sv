...
//   Same shape as pe (operand isolation -> pe_array_nr4sd_bfp -> acc_array_bfp, plus the
...
//       (a_dp8 and the two recoded B digit buses b_nr4sd_dp8/b_booth_dp8 - an
//       all-zero code is digit 0 on both, so a masked lane multiplies by zero)
//       AND the dispatched exponent operands (exp_a_dp8/exp_b_dp8) to zero. As
//       in pe, the PE clock is gated externally by the same enable, so
//       the mask keeps the pe_array_nr4sd_bfp logic before the first PE register (the
...
//
//   Pipeline: 3 stages (disp input reg upstream, pe_array_nr4sd_bfp L0 reg, acc_array_bfp
...
module pe_nr4sd_bfp #(
    ...
    localparam int B_NR4SD_DP8_WIDTH = 8 * CODE_WIDTH,
    localparam int B_BOOTH_DP8_WIDTH = 8 * BSEL_WIDTH,
    ...
    logic [B_NR4SD_DP8_WIDTH-1:0] b_nr4sd_dp8_m [0:NUM_DP8-1];
    logic [B_BOOTH_DP8_WIDTH-1:0] b_booth_dp8_m [0:NUM_DP8-1];
    ...
    pe_array_nr4sd_bfp pe_array_nr4sd_bfp_i (
        .clk_i        (clk_i),
        .rst_ni       (rst_ni),
        .a_dp8_i      (a_dp8_m),
        .b_nr4sd_dp8_i(b_nr4sd_dp8_m),
        .b_booth_dp8_i(b_booth_dp8_m),
