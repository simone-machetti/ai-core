...
//
// Description:
//   Beta array, square variant - the per-column B-only correction generator. It
//   is pe_array_sqr with the A operand removed and the 16 DP8 cores swapped to
//   dp_8_beta_sqr: identical 4-level crossed CPR-4:2 tree, complex-mode block
//   negate, widths and taps. It reduces the 16 per-DP8 beta square-sums
//   BETA_DP8 (each = 2^4*sum (B-8*au)^2 + sum (B-8)^2, the removed A operand's
...
//   unsigned L0 hi shift, the single L0 register, and the tap slicing. Node =
//   18 / 26 / 30 / 38 / 39 over DP8..L3; taps 19 / 30 / 38 / 39. Idle DP8s are
//   forced to a real zero via zero_i (the low block's fixed -8 would otherwise
//   leak). Per column of the grid, fanned into every PE in that column. See
//   pe_array_sqr for the tree.
// -----------------------------------------------------------------------------

`timescale 1 ns/1 ps

module pe_array_beta_sqr #(
