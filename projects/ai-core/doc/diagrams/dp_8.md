...
> partial product, so it has `CPR 8:2` ×3, shifts `<< 2` **and** `<< 4`, a `CPR 6:2`, and a **20-bit**
...
| `sum_o`         | out | 20     | Sum row of the carry-save result.                                |
| `carry_o`       | out | 20     | Carry row; `sum_o + carry_o = Σ aₖ·bₖ` (mod `2^20`).             |
...
`u×u` corner `8·255·15` and the `u×s` corner `8·255·(−8)`); the final 6:2 compressor adds two guard
bits (`EXT = 2`) on top of its 18-bit aligned rows, giving a **20-bit** output (16-bit value + 4 guard
bits). The extra guard bits are required — a redundant carry-save pair loses its top carry if it is
truncated to the resolved-value width, so the pair would no longer be sign-consistent. See
[`dp_8`](../../rtl/dp_8.sv) for the per-stage width breakdown.
