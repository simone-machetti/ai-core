...
| **`acc_array_sqr.sv`** (variant)                                                                                    | **built**          | all-additive resolve, per lane: triple tap mux (PE/`−α`/`−β` = 6 rows) + acc mux (seed/feedback, `<<1` on its output) + const mux (`CH`/`CL`/`RH`/`RL`, `RH=sign(RL)`) → **CPR 8:2** (`EXT=3`) → `add_n` → **`÷2`** (arith `>>1` + `H→L` cross-bit) → 20-bit reg. No subtractor/carry-in; the `<<1` keeps `acc_i`/`out` native and the register at the true value. Same 3 pipeline stages as the baseline (PE/α/β run in parallel). Verified bit-exact vs the baseline matmul (all 11 modes, single-shot + accumulation). See [acc_array_sqr — the all-additive resolve](#acc_array_sqr--the-all-additive-resolve). |
| **`pe_sqr.sv`** (variant)                                                                                           | **built**          | the square PE core — `pe_array_sqr` + `acc_array_sqr` + 2 acc pipeline regs, like `pe`, but takes the shared `−α`/`−β` taps and the `C`/`c_neg` const as inputs. `en_i` AND-masks A, B **and the `−α`/`−β` taps** for a fully-quiet gated PE; only the 2 acc regs pipeline internally.                                                                                                                                                                                                                                                                                                                              |
| **`ctrl_sqr.sv`** (variant)                                                                                         | **built**          | the square mode decoder (the `ctrl` analogue): `mode →` sel_a/sel_b, is_signed_a/b, **zero**, **neg**, sel_shift, sel_out, **sel_const**, prop_carry. Drops `ctr_l`/`ctr_h`; stage-1 controls combinational, stage-2 (`sel_shift[2:1]`, `sel_out`, `sel_const`, `prop_carry`) registered once.                                                                                                                                                                                                                                                                                                                      |
| **`top_NxN_sqr.sv`** (variant)                                                                                      | **built**          | the square N×N grid — `ctrl_sqr` + one shared `const_sqr` (4-bit `mode` registered once ahead, `c`/`c_neg` outputs once after — two register delays, one register cheaper than registering the wide outputs twice) + per-row `{disp_array_a_sqr + pe_array_alpha_sqr}` + per-col `{disp_array_b_sqr + pe_array_beta_sqr}` + N² `pe_sqr` + clock gating (`icg`, α/β generators share the row/col gate). Same interface/behaviour as `top_NxN`. Verified bit-exact vs `top_NxN` at full streaming throughput: all 11 modes, single-shot + accumulation + rectangle scaling, 0 mismatches, `-Wall` clean.                                                                                                                                                                                |
...
- `− 2N` — the block-negate deferral (§4), tree-weighted, on the **HW-negated Re** outputs only (modes 10/11; `2N = 34`, `68`). Mode 12 is **SW pre-negated** — no `comp_n`, so the `im·im` centering does *not* cancel in Re; its Re uses `c_o` (full `C_real+4`) like the Im half, and its `c_neg_o` is left unused.
...
(`c_neg_o` is added only on the **HW-negated** Re outputs — modes 10/11. Mode 12's `+4` entry is **unused**: being SW pre-negated, its Re takes `c_o` like a real-mode half. So `acc_array_sqr` selects `c_neg` only for modes 10/11; mode 12 uses the `c_o` const pattern.)

### acc_array_sqr — the all-additive resolve

Same 8-lane / lane-pair-fusion shape as the baseline `acc_array` (even lane = high half `[39:20]`, odd = low `[19:0]`; L0 single-lane), but each lane's tap mux selects a **triple** and the resolve is a pure carry-save **add** — no subtractor, no carry-in injection. Per lane:

- **Tap mux** (`sel_out_i`, over the 4 read levels): selects the PE, `−α` and `−β` carry-save taps at the mode's level → **6 rows** (each sum/carry pair windowed to 20-bit).
- **Acc mux** (`sel_acc_i`): external seed `acc_i` (first cycle) or register feedback, then a **`<<1`** on its output so the accumulated term enters at 2×.
- **Const mux** (`sel_const_i`, encoded from `ctrl`): the per-mode `C` from [const_sqr](../../../rtl/const_sqr.sv) — `CH`/`CL` (`c_o` hi/lo) on Im/real outputs, `RH`/`RL` (`c_neg` hi/lo) on the **HW-negated** Re outputs, with `RH = sign(RL)` (the Re constant fits the low 20; its high half is just the sign-extension). `sel_const_i` picks one of four patterns: `0` = L0 (mode 1, all `CL`), `1` = `c_o` on every half (real fused **and mode 12** — SW pre-negated, so its Re takes `c_o` too), `2` = complex L1 (mode 10, Re → `c_neg`), `3` = complex L2 (mode 11, Re → `c_neg`). So `c_neg` reaches only lanes 0/1/4/5 (mode 10) and 2/3 (mode 11):

  | lane  | H/L | uses `c_neg` in | const inputs |
  | ----- | --- | --------------- | ------------ |
  | 0,2,4 | H   | 10 / 11         | `CH, CL, RH` |
  | 1,3,5 | L   | 10 / 11         | `CL, RL`     |
  | 6     | H   | —               | `CH, CL`     |
  | 7     | L   | —               | `CL`         |

- **CPR 8:2** (`EXT=3`) folds the 8 rows `{6 taps, C, 2·acc}` → 2 rows; **`add_n`** resolves them + the fusion carry into the 20-bit window (`= 2·R_new`).
- **`÷2`** — an **arithmetic `>>1`** between `add_n` and the register, with an `H→L` cross-lane bit (the high lane's shifted-out LSB fills the low lane's MSB on fused outputs). The register holds the **true** result: `R_new = ½(tap + 2·acc) = ½·tap + acc` — no decay, no 2× register, and `acc_i`/`out` stay in native units (the `<<1` doubles the fed-back/seed term, the `>>1` undoes it).
- **Fusion carry** (`prop_carry_i`, `gate_n`): the low→high inter-lane carry as in the baseline; the `<<1`'s low-lane overflow rides this same chain (only the `÷2` needs its own `H→L` wire).

Two shifts total, both cheap: **`<<1` at the acc-mux output** (protects the running sum / seed from the `÷2`) and the **`÷2` `>>1`** after `add_n`. Widths: register 20-bit (spec: no overflow); CPR 8:2 `EXT=3` (23-bit), `add_n`/`gate_n` carry 3-bit (verify `+1` headroom for the `2·acc` row). **Pipeline = the same 3 stages as the baseline** — dispatchers register → PE/α/β L0 registers *in parallel* → acc register — so no added latency vs `disp → pe_array → acc_array`.
...
- **`acc_array_sqr`** (Gate 5): `tb_acc_array_sqr` wires the whole square path — `disp_array_{a,b}_sqr` → `pe_array_sqr` ∥ `pe_array_alpha_sqr` ∥ `pe_array_beta_sqr` → `const_sqr` → `acc_array_sqr` — and checks `pe_out` as a **plain matrix multiply**, reusing the baseline `tb_acc_array` golden / packing / reconstruction verbatim (the square path is bit-exact to the multiply path, so `pe_out` needs no rescale). All 11 modes × 2000 corner-biased vectors, single-shot (`acc_i=0`) **and** accumulation (native-unit seed + 8 feedback iterations), 0 mismatches, `-Wall` clean. This pass caught the mode-12 const subtlety (SW pre-negation → Re uses `c_o`, not `c_neg`).
- **`top_NxN_sqr`** (Gate 6): `tb_top_NxN_sqr` reuses the baseline `tb_top_NxN` streaming bench verbatim (the square grid shares the interface and is bit-exact) — distinct A per row, B per column, `pe[r][c] == A[r]·B[c]`, driven at **full pipeline throughput** (a fresh operand every clock, checked against a golden delayed by `D = LAT−1 = 2` via a ring buffer). All 11 modes, three streaming passes — single-shot, accumulation (per-PE seed + `NUM_ACC` **distinct** streamed tiles, `seed + Σ golden(tile)`), and **rectangle scaling** (`en_row`/`en_col`; disabled PEs clock-gated to hold), N=2, **0 mismatches**, `-Wall` clean. Streaming is the stronger grid check: it proves the shared −α/−β generator pipelines stay aligned with each PE's product **cycle by cycle** at full throughput. The scaling pass needs a clean reset deassertion (off the clock edge): unlike the multiply grid, a gated square PE latches `½(0−2−2)=−2` from the pipeline-empty transient (α/β generators emit `~0=−2`) if reset races the edge — the disabled-PE-hold behaviour is otherwise identical.
- **Global 3-way equivalence** (pre-synthesis): `tb_top_NxN_global` instantiates **both** `top_NxN` and `top_NxN_sqr` in one bench, drives them with the exact same streaming inputs every clock, and compares three results at every PE output — golden vs baseline vs square (`cmp3` flags `BAS!=GOLD` / `SQR!=GOLD` / `BAS!=SQR`). It is the streaming `tb_top_NxN` with the two DUTs merged (same tables/golden/packing, three passes). Proves `golden == baseline == square` for identical inputs, N=2, all 11 modes × 3 passes, **0 mismatches**, `-Wall` clean.
...
5. ✅ **`acc_array_sqr` + `tb_acc_array_sqr`** (Gate 5) — all-additive resolve `PE + (−α) + (−β) + C` (triple tap mux, acc-mux `<<1`, const mux from `const_sqr`) → CPR 8:2 → `add_n` → `÷2` (arith `>>1` + `H→L` cross-bit) → 20-bit reg; same 3 pipeline stages as the baseline. Verified as the **whole-path equivalence oracle** (disp → PE∥α∥β → const → acc == baseline matmul), all 11 modes, single-shot + accumulation, 2000 corner-biased vectors/mode, 0 mismatches, `-Wall` clean. See §7 and [acc_array_sqr — the all-additive resolve](#acc_array_sqr--the-all-additive-resolve).
6. ✅ **`pe_sqr` + `ctrl_sqr` + `top_NxN_sqr` + `tb_top_NxN_sqr`** (Gate 6) — the N×N grid: `ctrl_sqr` (mode decode) + one shared `const_sqr` (mode registered once ahead, outputs once after) + per-row `{disp_a_sqr + α gen}` + per-col `{disp_b_sqr + β gen}` + N² `pe_sqr` + `icg` clock gating (α/β share the row/col gate). Verified `== top_NxN` (baseline `tb_top_NxN` reused) at **full pipeline throughput** (fresh operand every clock, pipeline-delayed golden) — all 11 modes, streaming single-shot + streaming accumulation (`seed + Σ` distinct tiles) + rectangle scaling, N=2, 0 mismatches, `-Wall` clean.
