...
- Equivalence oracle for the full path (grid): **bit-exact match to `top_NxN`** for identical inputs, all modes, single-shot + accumulation, corner-biased extremes. Golden = the existing per-mode matmul model, extended to compute α/β and the `S`/÷2 path.
