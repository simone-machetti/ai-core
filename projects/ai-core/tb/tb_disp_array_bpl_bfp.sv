...
//   disp_array_b_bpl_bfp. For each of the 11 operating modes it drives the mode's
...
`endif
    ...
    initial begin
        $display("\nStarting disp_array_bpl_bfp verification (%0d modes x (%0d random + ramp))...\n",
                 NUM_MODE, NUM_RAND);
`ifdef VCD
        $dumpfile("activity.vcd");
        $dumpvars(0, tb_disp_array_bpl_bfp);
`endif
...
`ifdef VCD
        $dumpoff;
`endif
        $display("\ndisp_array_bpl_bfp: all %0d modes x (%0d random + ramp) tests PASSED!\n",
                 NUM_MODE, NUM_RAND);
        $finish;
    end
