...
//
// Description:
//   Self-checking testbench for dp_8_bpl_bfp. Drives NUM_RAND random (a, b)
...
//   sums from the raw 4-bit nibbles exactly as gate_b_n_bpl_bfp does - so the
...
`endif
    ...
    initial begin
        $display("\nStarting dp_8_bpl_bfp verification (LANES=%0d WIDTH_A=%0d WIDTH_B=%0d OUT_WIDTH=%0d)...\n",
                 LANES, WIDTH_A, WIDTH_B, OUT_WIDTH);
`ifdef VCD
        $dumpfile("activity.vcd");
        $dumpvars(0, tb_dp_8_bpl_bfp.dp_8_bpl_bfp_i);
`endif
...
`ifdef VCD
        $dumpoff;
`endif
        $display("dp_8_bpl_bfp: all %0d random + corner tests PASSED!\n", NUM_RAND);
        $finish;
    end
