...
//
// Description:
//   Self-checking testbench for cpr_w_n. Drives NUM_RAND random input vectors
...
module tb_cpr_w_n #(
    ...
    initial begin
        $display("\nStarting cpr_w_n verification (IN_WIDTH=%0d IN_SIZE=%0d IS_SIGNED=%0d OUT_WIDTH=%0d)...\n",
                 IN_WIDTH, IN_SIZE, IS_SIGNED, OUT_WIDTH);
        $dumpfile("activity.vcd");
        $dumpvars(0, tb_cpr_w_n.cpr_w_n_i);
        ...
        $dumpoff;
        $display("cpr_w_n: all %0d random + corner tests PASSED!\n", NUM_RAND);
        $finish;
    end
