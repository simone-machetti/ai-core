...
//
// Description:
//   Self-checking testbench for dp_8_bpl_b_bfp. Drives NUM_RAND random (a, b)
...
//   operand preparation - it derives the 9-bit exact values and the 10-bit pair
//   sums from the raw 8-bit lanes exactly as gate_n_bpl_bfp does - so the DUT is
//   exercised through the same contract it sees in the grid.
//
...
//   re-aligns the pair, so a merely resolving output is not enough. Lanes are
//   biased toward the extreme values (most-negative / max-positive / all-ones)
//   so the compressor guard bits are exercised at their bounds.
...
module tb_dp_8_bpl_b_bfp #(
    ...
    localparam int A_WIDTH   = WIDTH_A + 1;
    localparam int SUM_WIDTH = WIDTH_A + 2;
    ...
    logic [  A_WIDTH-1:0] a_ext   [  0:LANES-1];
    logic [SUM_WIDTH-1:0] a_sum_v [0:NUM_BLK-1];
    ...
    task automatic check(input bit sgn_a, input bit sgn_b);
        ...
        for (int k = 0; k < LANES; k++) begin
            a_ext[k] = {sgn_a & a_v[k][WIDTH_A-1], a_v[k]};
        end
        for (int j = 0; j < NUM_BLK; j++) begin
            a_sum_v[j] = SUM_WIDTH'(SUM_WIDTH'($signed(a_ext[2*j+0])) +
                                    SUM_WIDTH'($signed(a_ext[2*j+1])));
        end
        #1;
        exp  = 0;
...
`endif
    ...
    initial begin
        $display("\nStarting dp_8_bpl_b_bfp verification (LANES=%0d WIDTH_A=%0d WIDTH_B=%0d OUT_WIDTH=%0d)...\n",
                 LANES, WIDTH_A, WIDTH_B, OUT_WIDTH);
`ifdef VCD
        $dumpfile("activity.vcd");
        $dumpvars(0, tb_dp_8_bpl_b_bfp.dp_8_bpl_b_bfp_i);
`endif
...
`ifdef VCD
        $dumpoff;
`endif
        $display("\ndp_8_bpl_b_bfp: all %0d random + corner tests PASSED!\n", NUM_RAND);
        $finish;
    end
