...
//
// Description:
//   Self-checking testbench for pe_array_beta_sqr, driven through the square B
//   dispatcher (disp_array_b_sqr), mirroring tb_pe_array_sqr's disp -> array
//   structure. For each of the 11 modes it pushes NUM_RAND corner-biased random
//   256-bit B operands, lets the dispatcher center/idle-zero them, and checks
//   every tap at the mode's read level against a golden that recomputes the
//   tree: BETA_DP8 per DP8 from the dispatched (centered) B operand, the removed
//   A-high -8 bias (is_signed_a) on the high block and a fixed -8 on the low
//   block (idle-zeroed by zero_i), the complex-mode block negate (a negated
//   block resolves to -BETA_DP8-2, the deferred +2 being acc_array_sqr's job),
//   then the crossed 4-level weighted sum. Reports a fatal error on any mismatch.
//
//   BETA_DP8 = sum_k 16*(B_k - 8*au)^2 + (idle ? 0 : (B_k - 8))^2, au =
//   ~is_signed_a. This verifies the beta DP8 bias (gate_n_sqr high / gate_n_beta_
//   sqr low, incl. the idle-zero) + the shared tree; the +2 and the alpha/beta/C
//   reconstruction are the accumulator's concern (later gate).
//
// Parameters:
...
module tb_pe_array_beta_sqr #(
    ...
    function automatic longint bdp8(input logic [B_DP8_WIDTH-1:0] b, input logic isa, input logic zero);
        longint s;
        s = 0;
        for (int ln = 0; ln < LANES; ln++) begin
            longint bb, bah, bal;
            bb  = longint'($signed(b[ln*B_ELEM +: B_ELEM]));
            bah = isa  ? bb : bb - 8;
            bal = zero ? 0  : bb - 8;
            s  += 16*(bah*bah) + (bal*bal);
        end
        return s;
    endfunction
    ...
    task automatic check(input int mi);
        ...
        for (int i = 0; i < NUM_DP8; i++) begin
            sd[i]  = bdp8(b_dp8[i], is_signed_a[i], zero_dp8[i]);
        ...
        for (int node = 0; node < (1 << (3-lvl)); node++) begin
            ...
            // taps are one's-complemented (module emits -beta): read = -tree - 2
            case (lvl)
                0:       exp = -l0e[node] - 2;
                1:       exp = -l1e[node] - 2;
                2:       exp = -l2e[node] - 2;
...
`endif
    ...
    initial begin
        $display("\nStarting pe_array_beta_sqr verification (%0d modes x %0d random)...\n", NUM_MODE, NUM_RAND);
`ifdef VCD
        $dumpfile("activity.vcd");
        $dumpvars(0, tb_pe_array_beta_sqr.pe_array_beta_sqr_i);
`endif
...
`ifdef VCD
        $dumpoff;
`endif
        $display("\npe_array_beta_sqr: all %0d modes x %0d random tests PASSED!\n", NUM_MODE, NUM_RAND);
        $finish;
    end
