...
make pnr PROJECT=ai-core TOP_LEVEL=pe OUT_DIR=pe_pnr NETLIST_DIR=pe_syn CLK_PERIOD_NS=$CLK \
    CORE_UTIL=40 ASPECT_RATIO=1.0 CORE_MARGIN=2 PLACE_DENSITY=0.60 MAX_ROUTE_LAYER=M6 \
    CLK_UNCERTAINTY_PS=0 PNR_STEP=all PNR_THREADS=0 MACRO_DIRS=none FLOORPLAN=none PDN=none
...
make pnr PROJECT=ai-core TOP_LEVEL=top_NxN OUT_DIR=top_2x2_pnr NETLIST_DIR=top_2x2_syn CLK_PERIOD_NS=$CLK \
    CORE_UTIL=30 ASPECT_RATIO=1.0 CORE_MARGIN=2 PLACE_DENSITY=0.70 MAX_ROUTE_LAYER=M7 \
    CLK_UNCERTAINTY_PS=0 PNR_STEP=all PNR_THREADS=0 MACRO_DIRS="pe_pnr" \
    FLOORPLAN=projects/ai-core/scripts/floorplan_top_NxN.tcl MACRO_CHANNEL=20 PDN=none
