...
//   per-column B dispatcher recodes B once into two digits per nibble (an NR4SD+
//   code and a Booth window, 2 partial products per lane) and broadcasts the codes,
//   so the recode is paid O(N) instead of the O(N^2) it costs when it lives inside
//   every PE. ctrl's is_signed_b_o is routed to the DISPATCHERS instead of the PEs,
//   where it selects the cross-nibble carry of the 2-PP scheme.
//
//   exponent sideband. Tiles N*N pe_nr4sd_bfp cores; operand A (mantissa + exponent)
...
//     - disp_array_b_nr4sd_bfp + disp_array_exp_b_bfp : one pair per column, the
//       dispatcher also owning the NR4SD+ recoders (shared clk_b),
...
module top_NxN_nr4sd_bfp #(
    ...
    logic [B_NR4SD_DP8_WIDTH-1:0] b_nr4sd_dp8_col [0:NUM_COL-1][0:NUM_DP8-1];
    logic [B_BOOTH_DP8_WIDTH-1:0] b_booth_dp8_col [0:NUM_COL-1][0:NUM_DP8-1];
