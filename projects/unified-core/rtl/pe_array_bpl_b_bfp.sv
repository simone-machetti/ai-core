...
//
// Description:
//   BFP-capable PE array, bit-plane B build - the pe_array_bfp variant whose 16
//   DP8 cores are dp_8_bpl_b_bfp. Same 4-level crossed carry-save reduction tree,
//   same in-tree BFP alignment and exponent max tree, same taps at every level.
//
//   The bit-plane operand contract changes two things on the interface: a_dp8_i
//   carries 8 x 9-bit exact signed lane values and the added a_sum_dp8_i the
//   4 x 10-bit pairwise sums, both formed once per grid row by
//   disp_array_a_bpl_b_bfp; and is_signed_a_i is gone, since A's signedness is
//   already resolved in the dispatcher and never has to reach the PEs.
//   is_signed_b_i remains, feeding each DP8's weight-2^3 correction. b_dp8_i
//   carries the raw int4 lanes straight from disp_array_b.
//
...
//   L2 and L3 taps carry the accumulator's width. They are the widths of the
//   bit-plane A build, which is what lets both share acc_array_bpl_bfp.
//
...
module pe_array_bpl_b_bfp #(
    ...
    generate
        for (i = 0; i < NUM_DP8; i++) begin : gen_dp8
            ...
            logic [ SUM_WIDTH-1:0] a_sum_ln [0:NUM_A_SUM-1];
            ...
            for (ln = 0; ln < NUM_A_SUM; ln++) begin : gen_lane_sum
                assign a_sum_ln[ln] = a_sum_dp8_i[i][ln*SUM_WIDTH +: SUM_WIDTH];
            end
