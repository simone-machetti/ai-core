...
//
// Description:
//   BFP-capable PE array for the NR4SD variant - pe_array_bfp with the DP8
//   leaves swapped from dp_8 to dp_8_nr4sd_bfp. The tree body below the leaves
//   is UNCHANGED: dp_8_nr4sd_bfp keeps DP8_WIDTH at 20, so every node width,
//   tap width and guard bit is identical to baseline-BFP (taps 18/29/37/38) and
//   acc_array_bfp is reused as-is rather than forked the way the bit-plane
//   builds had to fork it.
//
//   B arrives ALREADY RECODED from disp_array_b_nr4sd_bfp as two digit buses per
//   DP8: b_nr4sd_dp8_i (8 lanes x 2-bit NR4SD+ code, the weight-1 digit) and
//   b_booth_dp8_i (8 lanes x 3-bit Booth window, the weight-4 digit) - 5 bits per
//   lane, 40 per DP8, against raw B's 4 and 32, two partial products per lane.
//   There is therefore no is_signed_b_i port: b's signedness is consumed by the
//   recoders in the dispatcher (as "top nibble of its element", which selects the
//   cross-nibble carry) and never reaches the PEs.
//
//   Same 16 DP8 cores and 4-level crossed carry-save reduction tree (see
//   pe_array), plus one align_cell_bfp before the CPR 4:2 of every L0, L2 and
//   L3 node, placed after the radix shift/extend on the width-matched CPR
//   operands. L1 never aligns: its merge recombines the H/L halves of the
//   same B block (same exponent by the format coherence rule), so only an
//   exponent max is forwarded there. With all exponents equal every aligner
//   is bit-transparent and the array behaves exactly like pe_array (the
//   pure-integer path); node, tap and guard widths are unchanged because an
...
module pe_array_nr4sd_bfp #(
    localparam int NUM_DP8           = 16,
    localparam int A_DP8_WIDTH       = 64,
    localparam int LANES             = 8,
    localparam int CODE_WIDTH        = 2,
    localparam int BSEL_WIDTH        = 3,
    localparam int B_NR4SD_DP8_WIDTH = LANES * CODE_WIDTH,
    localparam int B_BOOTH_DP8_WIDTH = LANES * BSEL_WIDTH,
    ...
    input  logic [B_NR4SD_DP8_WIDTH-1:0] b_nr4sd_dp8_i [0:NUM_DP8-1],
    input  logic [B_BOOTH_DP8_WIDTH-1:0] b_booth_dp8_i [0:NUM_DP8-1],
