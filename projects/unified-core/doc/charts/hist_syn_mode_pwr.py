...
#   for the five PE-grid variants - baseline (top_NxN), square (top_NxN_sqr),
#   BFP (top_NxN_bfp), square-BFP (top_NxN_sqr_bfp) and bit-plane-A BFP
#   (top_NxN_bpl_a_bfp) - at 8x8 and 16x16, split into
...
#   gate-level run per mode with 100 random operand sets. Bars are NORMALIZED to
#   the baseline averaged over the 11 modes - one reference for the whole figure,
#   so the per-mode spread of the baseline itself stays visible. Values are
#   inlined in DATA below and collected in doc/data/res_syn_mode_pwr.xlsx.
#   Colours follow
...
def draw(n):
    ...
    for i, m in enumerate(MODES):
        for v, off in zip(VARIANTS, offs):
            bottom = 0.0
            for sec in SECTIONS:
                val = DATA[v][str(n)][str(m)][sec] / ref
                if val > 0:
    ...
    ax.set_xlim(-0.65, len(MODES) - 0.35)
    ax.set_ylabel("Normalized dynamic power [mean Baseline = 1]")
    ax.set_title(f"Per-Mode Dynamic Power — Baseline / Square / Baseline-BFP / Square-BFP / "
                 f"Bit-Plane-A BFP / Bit-Plane-B BFP / NR4SD BFP, {n}x{n} Grid (ASAP7)")
    ...
    for m in MODES:
        i = MODES.index(m)
        vals = "  ".join(f"{v[:4]}={totals[v][i]:6.3f}" for v in VARIANTS)
