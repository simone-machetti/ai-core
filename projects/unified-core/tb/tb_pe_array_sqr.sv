...
//
// Description:
//   Self-checking testbench for pe_array_sqr, driven through the square
//   dispatchers (disp_array_a_sqr + disp_array_b_sqr), mirroring tb_pe_array's
//   disp -> array structure. For each of the 11 modes it pushes NUM_RAND
//   corner-biased random 256-bit operands, lets the dispatchers center/idle-zero
//   them, and checks every tap at the mode's read level against a golden that
//   recomputes the tree: S_DP8 per DP8 from the dispatched (centered) operands,
//   the complex-mode block negate (a negated block resolves to -S_DP8-2, the
//   deferred +2 being acc_array_sqr's job), then the crossed 4-level weighted
//   sum. Reports a fatal error on any mismatch.
//
//   This verifies the tree (weights, crossed pairing, the relocated negate, and
//   the widths); the +2 correction and the alpha/beta/C reconstruction are the
//   accumulator's concern (later gate).
//
// Parameters:
...
`endif
    ...
    initial begin
        $display("\nStarting pe_array_sqr verification (%0d modes x %0d random)...\n", NUM_MODE, NUM_RAND);
`ifdef VCD
        $dumpfile("activity.vcd");
        $dumpvars(0, tb_pe_array_sqr.pe_array_sqr_i);
`endif
...
`ifdef VCD
        $dumpoff;
`endif
        $display("\npe_array_sqr: all %0d modes x %0d random tests PASSED!\n", NUM_MODE, NUM_RAND);
        $finish;
    end
