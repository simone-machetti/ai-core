# Dot Product 8 (Bit-Plane-A BFP)

`dp_8_bpl_a_bfp` — the bit-plane build of the DP8 core. It computes the same `Σₖ aₖ·bₖ` as [dp_8](./dp_8.md) and returns it in carry-save form, but decomposes the multiplication over **the bit planes of A** instead of Booth-recoding B. Drop-in for `dp_8` inside [pe_array_bpl_a_bfp](./pe_array_bpl_a_bfp.md), with a wider operand contract and no `is_signed_b_i`.
...
The decisive part is where the fourth input comes from. The pair sums are a function of **B alone**, so they are precomputed once per grid column by [gate_n_bpl_bfp](./gate_n_bpl_bfp.md) inside [disp_array_b_bpl_a_bfp](./disp_array_b_bpl_a_bfp.md) and broadcast. The PE gets them for free; the adders are paid `N` times instead of `N²`.
...
**No `is_signed_b_i`** — B's signedness is resolved in [disp_array_b_bpl_a_bfp](./disp_array_b_bpl_a_bfp.md) and never reaches here. Combinational.
...
The output pair must be **sign-consistent** — `signext(sum_o) + signext(carry_o)` is the dot product, not merely its low bits — because [pe_array_bpl_a_bfp](./pe_array_bpl_a_bfp.md) sign-extends and re-aligns it. `cpr_w_n` drops any carry out of its top bit, so each stage needs guard bits: sign-consistency holds once `2^(W−1) > Σ|rows|`.

Every stage carries `EXT = 2`. The **tightest** is the per-column 4:2: a resolved B element spans `[−8, 15]`, so a pair sum spans `[−16, 30]` and four rows reach `Σ|rows| ≤ 120` against `2⁷ = 128` — only **6 % of margin**. That is close enough that sign-consistency is established by simulation rather than by construction; [tb_dp_8_bpl_a_bfp](../testbenches/tb_dp_8_bpl_a_bfp.md) drives corner-biased extremes for exactly this reason. The later stages are comfortable (75 % and 66 % of their bounds).

### Why the output is padded to 22 bits

The 9:2 produces 19 bits; a final [ext_n](./ext_n.md) sign-extends the pair to 22. The pad is **free** — sign extension is rewiring, zero cells — and it sizes the tree above: with `DP8_WIDTH = 22` the array's nodes land at 31/36/44/44, which lets the L1 tap carry its node in full and keeps the L2/L3 taps in the accumulator's 40-bit format. See [pe_array_bpl_a_bfp](./pe_array_bpl_a_bfp.md).

## Verification

[tb_dp_8_bpl_a_bfp](../testbenches/tb_dp_8_bpl_a_bfp.md) checks three properties on every vector — resolve, sign-consistency, and value-equivalence against a [dp_8](./dp_8.md) fed the same raw operands — under all four signedness combinations, with lanes biased toward the extremes so the tight column bound is actually reached.

Source: [dp_8_bpl_a_bfp.sv](../../rtl/dp_8_bpl_a_bfp.sv) — Testbench: [tb_dp_8_bpl_a_bfp.sv](../../tb/tb_dp_8_bpl_a_bfp.sv) — Diagram: [dp_8_bpl_a_bfp](../../doc/diagrams/dp_8_bpl_a_bfp.excalidraw)
