# Dispatch Array B (Bit-Plane-A BFP)

`disp_array_b_bpl_a_bfp` — the B-path dispatcher of the bit-plane variant, one per grid column. Routing is [disp_array_b](./disp_array_b.md) unchanged; what is added is one [gate_n_bpl_bfp](./gate_n_bpl_bfp.md) per DP8, so the column broadcasts **two** operand sets instead of one: the eight per-lane values already resolved to signed 5-bit, and the four pairwise sums at 6 bits.

## Purpose

The bit-plane [dp_8_bpl_a_bfp](./dp_8_bpl_a_bfp.md) needs, per lane pair and per bit plane, one of `{0, b₂ⱼ, b₂ⱼ₊₁, b₂ⱼ + b₂ⱼ₊₁}`. The pair sums are a function of **B alone**, so they can be hoisted out of the PE and computed once per grid **column**:
...
The second thing that moves here is **signedness**. Because the gate resolves each nibble to its exact signed value, `is_signed_b_i` is consumed in this module and never reaches the PEs — [pe_array_bpl_a_bfp](./pe_array_bpl_a_bfp.md) has no `is_signed_b_i` port at all.
...
`is_signed_b_i[j]` feeds only instance `j`'s gate. Once the gate has widened each nibble to its exact signed 5-bit value, the flag has done its job — nothing downstream needs it. This is the one interface change that ripples all the way up: [pe_bpl_a_bfp](./pe_bpl_a_bfp.md) and [pe_array_bpl_a_bfp](./pe_array_bpl_a_bfp.md) both drop `is_signed_b_i`, and in [top_NxN_bpl_a_bfp](../architectures/top_NxN_bpl_a_bfp.md) `ctrl`'s `is_signed_b` fans out to the `N` dispatchers instead of the `N²` PEs.
...
[tb_disp_array_bpl_a_bfp](../testbenches/tb_disp_array_bpl_a_bfp.md) drives all 11 modes with `ctrl`'s real control vectors and checks both outputs of every DP8 against a golden model of the whole chain — block select, H/L split, the per-int4 gate including the cross-half negate carry, the signedness-aware widening, and the four pairwise sums.

Source: [disp_array_b_bpl_a_bfp.sv](../../rtl/disp_array_b_bpl_a_bfp.sv) — Testbench: [tb_disp_array_bpl_a_bfp.sv](../../tb/tb_disp_array_bpl_a_bfp.sv) — Diagram: [disp_array_b_bpl_a_bfp](../../doc/diagrams/disp_array_b_bpl_a_bfp.excalidraw)
