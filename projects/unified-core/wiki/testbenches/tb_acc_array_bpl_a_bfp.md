# Accumulator Array (Bit-Plane-A BFP) Testbench

## Purpose

`tb_acc_array_bpl_a_bfp` verifies the **full bit-plane-A BFP PE datapath** with the integer datapath alongside as reference. The mantissa dispatchers feed both `pe_array → acc_array` (baseline) and [pe_array_bpl_a_bfp](../modules/pe_array_bpl_a_bfp.md) → [acc_array_bpl_bfp](../modules/acc_array_bpl_bfp.md), while the exponent dispatchers feed the BFP tree and accumulator. The check point is `pe_out` — the PE's real output, after tap resolve, accumulate and lane fusion.
...
```
make sim PROJECT=unified-core TOP_LEVEL=acc_array_bpl_bfp
```
...
This bench and [tb_top_NxN_bpl_a_bfp](./tb_top_NxN_bpl_a_bfp.md) are what caught the **non-canonical zero**: with `is_signed_a` left set on an idle DP8, [dp_8_bpl_a_bfp](../modules/dp_8_bpl_a_bfp.md)'s weight-2⁷ fold produces a carry-save pair with `s + c = 0` but `s ≠ 0`, and the BFP right-shift alignment in the accumulate loop destroys that cancellation — a one-LSB error that only appears once an aligner actually shifts. The fix lives in [top_NxN_bpl_a_bfp](../architectures/top_NxN_bpl_a_bfp.md), which masks `is_signed_a` from `ctrl`'s zero gate codes.

Result: **11/11 modes, all lanes, 0 mismatches**.

Source: [tb_acc_array_bpl_a_bfp.sv](../../tb/tb_acc_array_bpl_a_bfp.sv) — DUT: [acc_array_bpl_bfp](../modules/acc_array_bpl_bfp.md) (whole path from the dispatchers; reference `pe_array` → [acc_array](../modules/acc_array.md))
