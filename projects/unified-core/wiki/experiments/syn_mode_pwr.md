# Per-Mode Synthesis Power — Baseline / Square / Baseline-BFP / Square-BFP / Bit-Plane-A BFP / Bit-Plane-B BFP / NR4SD BFP

VCD-annotated dynamic power of the seven PE-grid variants measured **once per operating mode** on the complete 2×2 grids and assembled per component for 8×8 and 16×16.

## Purpose

[Synthesis Power](syn_pwr.md) drove all 11 modes into a single VCD and reported one averaged figure per variant. That average hides an enormous per-mode spread: the square's 8×8 margin runs from **−22.4 % (mode 6) to +1.4 % (mode 1)**, and the square-BFP's from **−14.1 % (mode 6) to +4.8 % (mode 1)** — so a single number both understates the win and hides the modes where the square is a net loss at the target grid size. The bit-plane variant makes the point far more sharply still: its grid-average is a flat −0.3 %, but per mode it runs from **−11.2 % to +23.1 %**. The NR4SD hybrid's −2.9 % average likewise splits into a **+5.6 % to −6.4 %** range whose sign is set by the operand format. This experiment measures each mode on its own.

## Method

As [Synthesis Power](syn_pwr.md) — per-component synthesis, blackbox-linked 2×2 grids, gate-level simulation, OpenSTA power with the resulting activity annotated — but pass C is run once per mode, 100 random operand sets in that mode alone. Seven variants × 11 modes = **77 gate-level runs**, each reusing its variant's one compiled binary (`MODE_SEL` / `NUM_STREAM` are read from `+mode` / `+vectors` at run time), because the Verilator build takes 10–18 min per variant while a simulation takes ~2 s — compiling per mode would turn the sweep into hours.

```
simv +mode=<m> +vectors=100          # one gate-level run per mode, per variant

make post-syn-dpa PROJECT=unified-core TOP_LEVEL=top_NxN_sqr_bfp \
...
Each variant's sweep is one `sweep <top> <bench> <grid> <blackbox list>` call in the script; the NR4SD line is `sweep top_NxN_nr4sd_bfp tb_top_NxN_nr4sd_bfp_pwr top_2x2_nr4sd_bfp "$BB_NR4SD_BFP"`, with the blackbox list of [Synthesis Area](syn_area.md).
...
| Mode | Square/Base 8×8 | Square/Base 16×16 | Sqr-BFP/Base-BFP 8×8 | Sqr-BFP/Base-BFP 16×16 | Bpl-A/Base-BFP 8×8 | Bpl-A/Base-BFP 16×16 | Bpl-B/Base-BFP 8×8 | Bpl-B/Base-BFP 16×16 | NR4SD/Base-BFP 8×8 | NR4SD/Base-BFP 16×16 | `sel_shift` |
| ---- | --------------- | ----------------- | -------------------- | ---------------------- | ------------------ | -------------------- | ------------------ | -------------------- | ------------------ | -------------------- | ----------- |
| 1    | **+1.4 %**      | −5.6 %            | **+4.8 %**           | **+0.8 %**             | **+23.1 %**        | **+23.0 %**          | −4.6 %             | −5.0 %               | **+5.6 %**         | **+5.6 %**           | `000`       |
| 2    | −10.9 %         | −17.9 %           | −4.6 %               | −8.5 %                 | −2.0 %             | −2.5 %               | −14.5 %            | −15.0 %              | −3.8 %             | −4.0 %               | `010`       |
| 3    | −4.0 %          | −11.4 %           | **+0.9 %**           | −3.0 %                 | **+1.2 %**         | **+0.8 %**           | −13.1 %            | −13.6 %              | −1.5 %             | −1.6 %               | `011`       |
| 5    | −12.8 %         | −18.9 %           | −5.6 %               | −8.4 %                 | **+22.8 %**        | **+22.9 %**          | −6.6 %             | −7.1 %               | **+4.3 %**         | **+4.3 %**           | `000`       |
| 6    | −22.4 %         | −28.5 %           | −14.1 %              | −17.1 %                | **+1.9 %**         | **+1.4 %**           | −16.8 %            | −17.6 %              | −4.2 %             | −4.5 %               | `010`       |
| 7    | −6.0 %          | −13.7 %           | **+0.9 %**           | −3.1 %                 | **+0.8 %**         | **+0.4 %**           | −13.8 %            | −14.3 %              | −0.8 %             | −0.9 %               | `011`       |
| 8    | −11.5 %         | −19.3 %           | −3.0 %               | −7.0 %                 | −11.2 %            | −11.8 %              | **−19.1 %**        | **−19.6 %**          | −6.1 %             | −6.3 %               | `111`       |
| 9    | −9.2 %          | −16.8 %           | −3.5 %               | −7.5 %                 | −11.2 %            | −11.7 %              | **−18.8 %**        | **−19.3 %**          | −6.3 %             | −6.5 %               | `111`       |
| 10   | −8.2 %          | −15.3 %           | −3.0 %               | −6.8 %                 | −1.7 %             | −2.2 %               | −14.2 %            | −14.7 %              | −3.7 %             | −3.8 %               | `010`       |
| 11   | −10.3 %         | −17.7 %           | −4.4 %               | −8.3 %                 | −3.7 %             | −4.2 %               | −16.2 %            | −16.7 %              | −3.9 %             | −4.0 %               | `010`       |
| 12   | −9.0 %          | −16.5 %           | −3.2 %               | −7.1 %                 | −11.1 %            | −11.6 %              | **−18.7 %**        | **−19.2 %**          | −6.4 %             | −6.5 %               | `111`       |
| mean | −8.8 %          | −16.1 %           | −2.7 %               | −6.5 %                 | +0.8 %             | +0.4 %               | **−14.2 %**        | **−14.7 %**          | −2.4 %             | −2.6 %               |             |

**Square vs baseline:** wins **10 of 11** at 8×8 (loses only mode 1) and **11 of 11** at 16×16. **Square-BFP vs baseline-BFP:** wins **8 of 11** at 8×8 (loses modes 1, 3, 7 — all narrowly) and **10 of 11** at 16×16 (loses only mode 1). **Bit-plane-A BFP vs baseline-BFP:** wins **6 of 11** at both sizes, and the spread is by far the widest — a 34-point range against the square's 24 and the square-BFP's 19. **Bit-plane-B BFP vs baseline-BFP:** wins **11 of 11** at both sizes, the only variant to sweep every mode, over a 14-point range (−4.6 % to −19.1 % at 8×8). **NR4SD BFP vs baseline-BFP:** wins **9 of 11** at both sizes, losing only modes 1 and 5, over a 12-point range (+5.6 % to −6.4 % at 8×8).

For the two square pairs the mean margins match the all-mode merged figures of [Synthesis Power](syn_pwr.md) to within 0.3 pp, cross-validating both experiments. The bit-plane pair does **not**: its unweighted per-mode mean is +0.8 % / +0.4 % against the merged run's −0.30 % / −0.75 %, a gap of ~1.1 pp. That is a consequence of the spread rather than a discrepancy — the merged VCD weights each mode by its actual toggle activity, and with a ±23 % range the unweighted mean and the activity-weighted one are simply different statistics. It is also a warning that no single number describes this variant. **Bit-plane B behaves like the square pairs rather than like bit-plane A**: its unweighted mean of −14.2 % / −14.7 % sits within 0.3 pp of the merged run's −14.5 % / −15.0 %, so for that variant a single number *is* usable. NR4SD sits between the two: its unweighted mean of −2.4 % / −2.6 % is within 0.5 pp of the merged run's −2.9 % / −3.0 %, close enough for the single number to stand — provided the two losing modes are named next to it.
...
| `sel_shift` | shifts enabled | modes        | Bpl-A margin at 8×8 | Bpl-B margin at 8×8 | NR4SD margin at 8×8 |
| ----------- | -------------- | ------------ | ------------------- | ------------------- | ------------------- |
| `000`       | 0              | 1, 5         | **+23 %**           | −4.6 … −6.6 %       | **+4.3 … +5.6 %**   |
| `010`       | 1              | 2, 6, 10, 11 | −3.7 … +1.9 %       | −14.2 … −16.8 %     | −3.7 … −4.2 %       |
| `011`       | 2              | 3, 7         | +0.8 … +1.2 %       | −13.1 … −13.8 %     | −0.8 … −1.5 %       |
| `111`       | 3              | 8, 9, 12     | **−11 %**           | **−19 %**           | **−6.1 … −6.4 %**   |
...
The NR4SD column sorts the same way, and there the mechanism is not a hypothesis but a property of the encoding — its DP8 hands up the same 20-bit row as Booth, so the tree is out of the picture. `sel_shift = 000` marks the two **whole-int4** modes (1 and 5, R8R4), where B is one signed nibble: for a signed nibble the baseline Booth DP8's third partial-product row is already a constant zero (the top digit recodes to 0, see [dp_8](../modules/dp_8.md)), so the hybrid removes nothing there and brings only its 25 % wider code bus — +5.6 % and +4.3 %. Every other mode slices B into a signed high nibble and an *unsigned* low nibble (R8R8, R16R8, R16R16 and the complex modes), which keeps the baseline's third row live, and there the two-row array wins: −3.7 … −4.5 % in the int8 modes, −0.8 … −1.6 % in the R16R8 pair, and the most — −6.1 … −6.5 % — in the int16 modes 8, 9, 12. The margin is keyed to the operand format, not to the tree, which is why the NR4SD column is the one whose sign a designer can predict from the mode table alone.
...
**NR4SD keeps a milder form of the pathology.** The same two no-shift modes, 1 and 5, have a **negative** per-tile saving (−0.0500 and −0.0240 mW), so no grid size ever recovers them — but the penalty is +5.6 % / +4.3 % rather than bit-plane A's +23 %, and it has a structural cause (the whole-int4 modes, above) rather than a tree effect. The other nine modes are positive, from +0.0105 mW (mode 7) to +0.0695 mW (modes 9, 12), against the cheapest dispatcher overhead of the three enlarged-dispatcher variants — 0.013–0.020 mW per row+column, a quarter of bit-plane A's. Seven of them cross over below N = 1, and the R16R8 pair at N = 1.07 (mode 3) and N = 1.87 (mode 7), so every mode it wins it already wins at 2×2:

| Mode | save/tile [mW] | dispatch per row+col [mW] | crossover N | asymptote |
| ---- | -------------- | ------------------------- | ----------- | --------- |
| 1    | **−0.0500**    | 0.0169                    | never       | 1.0555    |
| 2    | +0.0427        | 0.0195                    | **0.45**    | 0.9583    |
| 3    | +0.0178        | 0.0190                    | 1.07        | 0.9823    |
| 5    | **−0.0240**    | 0.0130                    | never       | 1.0425    |
| 6    | +0.0272        | 0.0151                    | **0.56**    | 0.9522    |
| 7    | +0.0105        | 0.0196                    | 1.87        | 0.9892    |
| 8    | +0.0657        | 0.0200                    | **0.30**    | 0.9350    |
| 9    | +0.0695        | 0.0200                    | **0.29**    | 0.9329    |
| 10   | +0.0410        | 0.0194                    | **0.47**    | 0.9598    |
| 11   | +0.0420        | 0.0180                    | **0.43**    | 0.9579    |
| 12   | +0.0695        | 0.0204                    | **0.29**    | 0.9324    |

Set next to bit-plane B, the per-tile savings are a third of its (+0.0695 against +0.2058 mW in mode 9) for a dispatcher cost that is smaller still — the NR4SD trade is safe everywhere it applies, it is simply shallow.
...
- **NR4SD is a mild, format-keyed version of the bit-plane-A story.** It wins 9 of 11 modes at every grid size — −4 … −6.5 % wherever B is sliced — and loses the two whole-int4 modes by +4 … +6 %, a property of the encoding (the baseline's third Booth row is already zero for a signed nibble), not of the grid, so it cannot be grown away. Its worst mode is a quarter of bit-plane A's penalty and its best mode a third of bit-plane B's win; against bit-plane B it loses in every mode, so as a power play it is the wrong choice, and its case rests on area (−12.4 % at 8×8, uniform across modes) with power as a small, workload-dependent side effect that an int4-heavy workload turns slightly negative.
...
| Pass                          | Wall time                                                                  |
| ----------------------------- | -------------------------------------------------------------------------- |
| A + B — synthesis             | ~2 min per variant (NR4SD measured), ~15 min for all seven                 |
| C — Verilator build           | ~10 min per variant for the earlier six, **18 min** measured for NR4SD     |
| C — gate-level simulations    | ~2 s each; 77 in about 3 min                                               |
| D — power runs                | **~1.7 min** each (NR4SD measured): 11 per variant ≈ 19 min, 77 ≈ 2.2 h    |

The NR4SD figures are from the 2026-09-09 sweep on a 16-core machine; the whole NR4SD pass C + D took 40 min.
